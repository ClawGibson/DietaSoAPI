        ...
        try {
            ...
            if (!datosDeUsuario)
                return res.status(500).json({
                    success: true,
                    message:
                        'El usuario no tiene datos de indicadores de sueño todavia',
                });
...
router.post('/individual', async (req, res) => {
            ...
            try {
                if (infoUsuario)
                    return res.status(500).json({
                        success: false,
                        message:
                            'Datos de indicadores de sueño de Usuario ya registrados',
                    });
            } catch (err) {
                return res.status(500).json({
                    success: false,
    ...
    try {
        ...
        return res.status(500).json({
            success: false,
            message:
                'Ocurrió un error al guardar los datos de indicadores de sueño',
        });
    }
});
