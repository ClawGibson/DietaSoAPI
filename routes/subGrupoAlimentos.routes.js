...
router.get('/', async (req, res) => {
    ...
    if (!subGruposDeAlimentos) res.status(500).json({ success: false });
...
router.put('/:id', async (req, res) => {
    const nuevoSubGrupo = await subGrupo.findOneAndUpdate(
        req.params.id,
        {
            subGrupoDeAlimento: req.body.subGrupoDeAlimento,
        },
    ...
    if (!nuevoSubGrupo)
        return res
            .status(404)
            .send('El sub grupo no se encontró o no se pudo editar :c');
...
router.delete('/:id', async (req, res) => {
    ...
    if (!subGrupo)
        return res
            .status(400)
            .send('No se encontró el sub grupo a eliminar :c');
